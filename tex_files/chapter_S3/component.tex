...
In order to use Media Management S3 Component in an X-Project App there's need to combinate five different X-Elements, for the server-side: \texttt{api-s3-upload}, \textt{api-s3-list}, \textt{api-s3-delete}, \textt{part-s3-list} and \textt{part-s3-upload}.
Each element provides the opportunity to use one of the basic function of Amazon S3 service. 

\subsubsection{\textt{api-s3-upload}}
\label{api-s3-upload}
The element named \textt{api-s3-upload} needs to upload new files 

\subsubsection{\textt{api-s3-list}}
\label{api-s3-list}
The element named \textt{api-s3-list} makes...

\subsubsection{\textt{api-s3-delete}}
\label{api-s3-delete}
The element named \textt{api-s3-delete} makes...

\subsubsection{\textt{part-s3-list}}
\label{part-s3-list}
The element named \textt{part-s3-list} makes...

\subsubsection{\textt{part-s3-upload}}
\label{part-s3-upload}
The element named \textt{part-s3-upload} makes...
